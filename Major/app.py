...
# ---------- TRAIN ----------
        ...
        if df is not None:
            ...
            col1, col2 = st.columns(2)
            epochs_to_train = col1.slider("Training Epochs", min_value=10, max_value=200, value=50, step=10)
            learning_rate = col2.selectbox("Learning Rate", [0.01, 0.005, 0.001, 0.0001], index=1)
            ...
            if "model" not in st.session_state:
                ...
                with torch.no_grad():
                    ...
                    if id_score < 0.5:
                        ...
                        disease_index = diseases.index(selected_disease)
                        disease_prob = pred_out[0][disease_index].item()
                        
                        st.markdown("---")
                        if disease_prob > 0.5:
                            st.warning(f"**Clinical Alert**: Likelihood of {selected_disease} is High ({disease_prob:.1%})")
                            
                            # Probable Cause Analysis
                            causes = []
                            if selected_disease == "Heart Disease" and age > 50: causes.append("Age factor")
                            if selected_disease == "Heart Disease" and smoking == "Smoker": causes.append("Smoking history")
                            if selected_disease == "Heart Disease" and fatigue == "Yes": causes.append("Reported fatigue")
                            
                            if selected_disease == "Flu" and temp >= 100: causes.append("Fever")
                            if selected_disease == "Flu" and bodypain == "Yes": causes.append("Body aches")
                            
                            if selected_disease == "Asthma" and smoking == "Smoker": causes.append("Smoking history")
                            if selected_disease == "Asthma" and cough == "Yes": causes.append("Persistent cough")

                            if selected_disease == "Covid" and temp >= 99.5: causes.append("Fever")
                            if selected_disease == "Covid" and cough == "Yes": causes.append("Cough")
                            if selected_disease == "Covid" and sorethroat == "Yes": causes.append("Sore Throat")
                            if selected_disease == "Covid" and fatigue == "Yes": causes.append("Fatigue")
                            
                            if len(causes) > 0:
                                st.info(f"**Probable Causes for Flagging {selected_disease}:**\n- " + "\n- ".join(causes))

                        else:
                            st.info(f"**Clinical Status**: Likelihood of {selected_disease} is Low ({disease_prob:.1%})")
